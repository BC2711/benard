@php
    $support = \App\Models\SupportSection::first();
    $loanAgreementAvailable = filled(data_get(
        \App\Models\Setting::query()->where('group', 'documents')->where('key', 'loan_agreement_path')->first()?->value,
        '0',
    ));
@endphp
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl p-6 shadow-xl animate-fade-in-up h-full flex flex-col" style="animation-delay: 0.1s;">
                        @if ($loanAgreementAvailable)
                            <div class="mb-6 pb-4 border-b border-gray-200">
                                <a href="{{ route('loan-agreement.download') }}"
                                    ...
                                    <i class="fas fa-download text-sm opacity-80 group-hover:translate-y-0.5 transition-transform"></i>
                                </a>
                                <p class="text-xs text-gray-400 text-center mt-2">Preview our standard loan terms before applying</p>
                            </div>
                        @endif
